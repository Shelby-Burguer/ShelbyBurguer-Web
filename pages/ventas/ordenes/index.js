...
const OrdenesPage = () => {
    ...
    const onFilter = (e, a, sf) => {
        ...
        if (value.length === 0) {
            ...
            const filtered = dataViewValue.filter((product) => {
                return product[a].toLowerCase().includes(value.toLowerCase());
            });
    ...
    const dataViewHeader = (
        <div className="flex flex-column md:flex-row md:justify-content-between gap-2">
            <Dropdown value={globalFilterValue} options={categoryOptions} optionLabel="label" placeholder="Buscar por categoría" onChange={(e) => onFilter(e, 'category', setGlobalFilterValue)} />
            <span className="p-input-icon-left">
                <i className="pi pi-search" />
                <InputText value={filterName} onChange={(e) => onFilter(e, 'name', setFilterName)} placeholder="Buscar por nombre" />
            </span>
